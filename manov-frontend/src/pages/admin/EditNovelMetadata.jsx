...
const EditNovelMetadata = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                ...
                setSelectedGenres(
                    novelRes.data.genres
                        ? novelRes.data.genres.map((g) => g.id)
                        : []
                );
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            await adminService.updateNovel(formData.id, {
                ...formData,
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <p className="ml-1 mt-1 text-[10px] text-gray-400">
                                        Warning: Changing this will change the
                                        URL. Make sure no broken links occur.
                                    </p>
                                </div>
                                                            ...
                                                            <button
                                                                onClick={async () => {
                                                                    if (
                                                                        ...
                                                                        return;
                                                                    try {
                                                                        await adminService.deleteChapter(
                                                                            chapter.id
                                                                        );
                                                                        toast.success(
                                                                            'Deleted'
