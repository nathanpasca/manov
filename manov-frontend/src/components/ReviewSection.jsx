...
const ReviewSection = ({ novelId }) => {
    ...
    const fetchReviews = async () => {
        try {
            setLoading(true);
            const res = await socialService.getReviews(novelId, 0, LIMIT);
            setReviews(res.data);
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setSubmitting(true);
            await socialService.postReview(novelId, score, content);
            setScore(0);
            setContent('');
    ...
    const handleUpdate = async (reviewId) => {
        try {
            await socialService.updateReview(reviewId, editScore, editContent);
            setEditingId(null);
