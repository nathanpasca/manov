...
function CommentItem({ comment, user, onDelete, onReply, replies }: CommentItemProps) {
    ...
    <div className="space-y-4">
      <div className="group flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {isReplying && (
            <form onSubmit={handleReplySubmit} className="relative mt-3">
              <textarea
                value={replyContent}
      ...
      {replies.map((reply) => (
        <div
          key={reply.id}
          className="ml-8 border-l-2 border-stone-100 pl-4 dark:border-white/5"
        >
          <div className="group flex gap-3">
            <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-stone-500 to-stone-700 text-xs font-bold text-white shadow-sm">
...
export default function CommentSection({ targetId, type = 'novel' }: CommentSectionProps) {
  const { user } = useAuth();
  ...
  const fetchComments = async (skip = 0, limit = LIMIT) => {
    try {
      const res = await api.getComments(endpoint, skip, limit);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const res = await api.postComment(endpoint, newComment);
  ...
  const handleReplySubmit = async (parentId: string, content: string) => {
    ...
    try {
      const res = await api.postComment(endpoint, content, parentId);
      const newData = Array.isArray(res) ? res : res.data;
      setComments((prev) => [newData, ...prev]);
  ...
  return (
    <div className="mx-auto mt-10 max-w-4xl">
      ...
      <div className="space-y-6">
        {loading ? (
          <p className="text-center text-stone-500">Loading comments...</p>
        ) : topLevel.length > 0 ? (
          <>
