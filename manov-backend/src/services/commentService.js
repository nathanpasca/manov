...
async function createComment(userId, commentData) {
  ...
  if (!parentId) {
    ...
    if (!data.novelId && !data.chapterId) {
      throw new Error(
        'A top-level comment must be associated with either a novel or a chapter.'
      );
    }
    if (data.novelId && data.chapterId) {
      throw new Error(
        'A top-level comment cannot be associated with both a novel and a chapter.'
      );
    }
    // Check if novel/chapter exists
    if (
      data.novelId &&
      !(await prisma.novel.findUnique({ where: { id: data.novelId } }))
    ) {
      throw new Error(`Novel with ID ${data.novelId} not found.`);
    }
    if (
      data.chapterId &&
      !(await prisma.chapter.findUnique({ where: { id: data.chapterId } }))
    ) {
 ...
 */
async function getCommentsForNovel(
  novelId,
  pagination = {},
  ...
  const totalCountPromise = prisma.comment.count({ where });
