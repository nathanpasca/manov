...
const validateChapterCreation = [
  ...
  body('chapterNumber')
    ...
    .isFloat({ min: 0 })
    .withMessage('Chapter number must be a non-negative number.') // Allow 0, 0.5, 1, 1.5 etc.
    .toFloat(),
  ...
  body('title')
    ...
    .trim()
    .isLength({ max: 255 })
    .withMessage('Chapter title must be at most 255 characters.'),
  body('wordCount')
    .optional({ checkFalsy: true })
    .isInt({ min: 0 })
  ...
  body('publishedAt')
    ...
    .isISO8601()
    .withMessage(
      'Published at must be a valid ISO 8601 date (YYYY-MM-DD HH:MM:SS or YYYY-MM-DD).'
    )
    .toDate(),
  body('translatorNotes')
    ...
    .trim()
    .isLength({ max: 5000 })
    .withMessage('Translator notes must be at most 5000 characters.'),
  body('originalChapterUrl')
    .optional({ checkFalsy: true })
    .trim()
    .isURL()
    .withMessage('Original chapter URL must be a valid URL.'),
  body('readingTimeEstimate')
    .optional({ checkFalsy: true })
    .isInt({ min: 0 })
    .withMessage('Reading time estimate must be a non-negative integer.')
    .toInt(),
...
const validateChapterUpdate = [
  ...
  body('chapterNumber')
    .optional()
    .isFloat({ min: 0 })
    .withMessage('Chapter number must be a non-negative number if provided.')
    .toFloat(),
  body('content')
    .optional()
    .trim()
    .notEmpty()
    .withMessage('Chapter content cannot be empty if provided.'),
  ...
  (req, res, next) => {
    ...
    if (req.method === 'PUT' && Object.keys(req.body).length === 0) {
      return res
        ...
        .json({
          message:
            'No data provided for update. At least one field must be present.',
        });
    }
    next();
  },
...
const validateNovelIdParamForChapters = [
  param('novelId')
    .isInt({ min: 1 })
    .withMessage('Novel ID in path must be a positive integer.')
    .toInt(),
  handleValidationErrors,
];
...
const validateChapterIdParam = [
  param('chapterId')
    .isInt({ min: 1 })
    .withMessage('Chapter ID in path must be a positive integer.')
    .toInt(),
  handleValidationErrors,
];
...
const validateChapterNumberParam = [
  param('chapterNumber')
    .isFloat({ min: 0 })
    .withMessage('Chapter number in path must be a non-negative number.')
    .toFloat(),
  handleValidationErrors,
];
