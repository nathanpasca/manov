...
const validateUserIdParamForAdmin = [
  param('userId')
    .isString()
    .withMessage('User ID must be a string (CUID).')
    .notEmpty()
    .withMessage('User ID cannot be empty.')
    .isLength({ min: 20, max: 30 })
    .withMessage('User ID format is incorrect (expecting CUID length).'),
...
const validateAdminUserUpdate = [
  body('displayName')
    ...
    .withMessage(
      'Display name must be between 1 and 50 characters if provided.'
    ),
  body('email')
    .optional()
    .trim()
    .isEmail()
    .withMessage('Must be a valid email address if provided.')
    .normalizeEmail(),
  body('isActive')
    .optional()
    .isBoolean()
    .withMessage(
      'isActive must be a boolean value (true or false) if provided.'
    ),
  body('isAdmin')
    .optional()
    .isBoolean()
    .withMessage(
      'isAdmin must be a boolean value (true or false) if provided.'
    ),
  body('preferredLanguage')
    ...
    .withMessage(
      'Preferred language code should be 2-5 characters if provided.'
    ),
  body('avatarUrl')
    .optional({ checkFalsy: true })
    .trim()
    .isURL()
    .withMessage('Avatar URL must be a valid URL if provided.'),
...
const validateAdminListUsersQuery = [
  ...
  query('isActive')
    .optional()
    .isBoolean()
    .withMessage('isActive filter must be a boolean (true or false).')
    .toBoolean(), // Converts "true"/"false" strings to boolean
  query('isAdmin')
    .optional()
    .isBoolean()
    .withMessage('isAdmin filter must be a boolean (true or false).')
    .toBoolean(),
  query('sortBy')
    .optional()
    .isIn(['username', 'email', 'createdAt', 'lastLoginAt', 'displayName'])
    .withMessage(
      "Invalid sortBy field. Allowed: 'username', 'email', 'createdAt', 'lastLoginAt', 'displayName'."
    ),
  query('sortOrder')
    .optional()
    .isIn(['asc', 'desc'])
    .withMessage("Sort order must be 'asc' or 'desc'."),
  query('username') // New filter
    .optional()
    .trim()
    .isString()
    .withMessage('Username filter must be a string.')
    .escape(), // Basic sanitization
  query('email') // New filter
    .optional()
    .trim()
    .isString()
