...
const validateLangQueryParam = [
  query('lang')
    .optional()
    .isString()
    .withMessage('Language query parameter (lang) must be a string.')
    .trim()
    .isLength({ min: 2, max: 10 })
    .withMessage(
      'Language code must be between 2 and 10 characters (e.g., en, pt-BR).'
    ),
...
const validatePaginationQuery = [
  ...
  query('limit')
    .optional()
    .isInt({ min: 1, max: 1000 }) // Increased max limit, e.g., to 1000
    .withMessage('Limit must be a positive integer, max 1000.') // Update message
    .toInt(),
